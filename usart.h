...
//reads usart until zero terminator is encountered (terminator is copied to buffer as well)
//NOTE: if the buffer fills before a zero is found, the string will not be terminated
size_t usart_read_string(char* buffer, size_t buffer_size);

//reads until a newline is found
//appends a zero terminator after the newline
//if a newline is not found, buffer_size - 1 characters will be read, and a zero terminator will be added
