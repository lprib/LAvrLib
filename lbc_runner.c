...
//the number of bytes at the start of a function in bytecode that are not actual code
//only 2 at this point in time to define local allocation size
#define FUNCTION_HEADER_SIZE 2

#define BINARY_OP_CASE(opcode, op) case opcode: \
context.stack_ptr -= 1; \
stackval_t opcode ## _val = context.stack_ptr[0] op context.stack_ptr[1]; \
context.stack_ptr[0] = opcode ## _val; \
break

unsigned int get_int_at_index(unsigned char* byte_array, int index);
...
void execute_function(ExecutionContext context, int function_index) {
	...
	int function_table_entry_index = FUNCTION_TABLE_OFFSET + FUNCTION_TABLE_ENTRY_SIZE * function_index;
	unsigned int function_start = get_int_at_index(context.bytecode, function_table_entry_index);
	
	unsigned int locals_allocation_size = get_int_at_index(context.bytecode, function_start);
	...
	while(1) {
		...
		switch(context.bytecode[instruction_pointer++]) {
			...
			case OP_SYSCALL:
			(*syscalls[next_bytecode_int(&context, &instruction_pointer)])(&context);
			break;
...
//consumes an int from the instruction stream, modifying instruction_pointer accordingly
//offset is the amount of bytes after the current instruction pointer
stackval_t next_bytecode_int(ExecutionContext* context, int* instruction_pointer) {
	stackval_t ret = (context->bytecode[*instruction_pointer]<<8) + context->bytecode[*instruction_pointer + 1];
